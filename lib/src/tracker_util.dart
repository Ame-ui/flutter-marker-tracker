...
class MarkerTracker {
  ...
  ///
  /// if [onGetOffset] give null value it mean
  ///
  ...
  static void calculateOffset(
    ...
    // }
    // if (shouldCalculate) {
    final bearing = _calculateBearing(centerLat, centerLng, trackLat, trackLng);
    ...
    if (bearing >= tr && bearing < br) {
      ...
      if ((bearing >= tl && bearing <= 360)) {
        final percentage =
            _calculatePercentWithDegree(degree: bearing, min: tl, max: 360) / 2;
